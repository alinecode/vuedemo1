	...
	export default {
		...
		methods: {
			...
			initws() {
				...
				// 多页面使用参考 https://blog.csdn.net/weixin_39531576/article/details/90485195
				...
				if(typeof(WebSocket) == "undefined") {
					...
					// bearer 
					var token = 'eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.eyJhdWQiOlsib2F1dGgyLXJlc291cmNlIl0sInVzZXJfbmFtZSI6IjEiLCJzY29wZSI6WyJhbGwiXSwib3JnYW5pemF0aW9uIjoiMUFM55qE5rWL6K-VIiwiZXhwIjoxNTg1MDM0OTczLCJhdXRob3JpdGllcyI6WyJST0xFX1VTRVIiXSwianRpIjoiNDgzMmQzMjMtZTg2Mi00MzFlLWJiNjUtY2JiYzE1YjQ1YTdlIiwiY2xpZW50X2lkIjoiemRlbW8ifQ.O7UsaRmc1YAbHeZDMD2QFO42qwp0BAUBuTfOnDYaw74DcPZxi8yTWDLoqh9nwQnvIYSAUodqlrUPiRZylvlvMzX7AH1IBH-x-XF18y9hHQ0ap0wR7Njevha-r8arFdMreHRTF_Ak0OEYekT20OrXRk-JalOVStfUREl3uf7xDfs'
					
//					this.socket = new WebSocket(this.path + aa,[token]);
					this.socket = new WebSocket('ws://192.168.9.80:9999/store-service/websocket/' + aa+'?'+'access_token='+token);
...
//					console.log(this.$AL);
					...
					this.$AL.getToken('/oauth/token',tok).then(function(reJson) {
						...
						window.localStorage.setItem('token', reJson.data.access_token);
						window.localStorage.setItem('tokenType', reJson.data.token_type);
						window.localStorage.setItem('refresh_token', reJson.data.refresh_token);
