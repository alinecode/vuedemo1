...
layui.define(['layer', 'laytpl', 'upload'], function(exports) {
	...
	var elemChatMain = ['<li {{ d.mine ? "class=layim-chat-mine" : "" }}>', '<div class="layim-chat-user"><img src="{{ d.avatar }}"><cite>', '{{# if(d.mine){ }}', '<i>{{ layui.data.date(d.timestamp) }}</i>{{ d.username||"佚名" }}', '{{# } else { }}', '{{ d.username||"佚名" }}<i>{{ layui.data.date(d.timestamp) }}</i>', '{{# } }}', '</cite></div>', '<div class="layim-chat-text">{{ layui.data.content(d.content||"&nbsp") }}</div>', '</li>'].join('');
	...
	var layimClose, popmin = function(content) {
		...
		return layer.open({
			...
			skin: 'layui-box layui-layim-min layui-layim-close layim-layer',
			shade: false,
			closeBtn: false,
			anim: 1,
			offset: ['150px', '890px'],
			resize: false,
			content: '<i class="fa fa-comments-o" style="font-size:25px;color:#72B8E9;"></i><span style="color:#C0E7FF">' + (content || cache.base.title || '我的通讯') + '</span>',
			move: '#layui-layim-close i',
	...
	'modules/layim/layim.css?v=2.092', 'skinlayimcss'
);